...
get_stat_samples = function(V, nu = rep(Inf, nrow(V)), n.mc.samples=1e5 ){
...
.LS_empirical_pvalue = function(LS.stat, V, nu = rep(Inf, nrow(V)), n.mc.samples=1e5){
	...
	stats = get_stat_samples(V, nu, n.mc.samples)
...
#' @export
#' @seealso \code{LS()}
LS.empirical = function(beta, stders, cor = diag(1, length(beta)), nu = rep(Inf, length(beta)), n.mc.samples=1e5){
