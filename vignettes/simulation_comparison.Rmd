...
S_MultiXcan.reg is proposed by [Barbeira, et al, 2019](https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1007889)
...
res = lapply( sig_array, function(sig){
  ...
  res = lapply(p_array, function(p){
    ...
    dcmp = eigen(cor(X))
    incl = dcmp$values[1] / dcmp$values < 300
    ...
    res = lapply( 1:n_sim, function(i){
    	...
    	if( runif(1) < 0.1) message("\r", "sig=", sig, " p=", p, " i=", i, "     ", appendLF=FALSE)
...
```{r make.plots, echo=FALSE, fig.width=12, fig.height=20, cache=FALSE}
...
fig1 = ggplot(df_FPR, aes(variable, FPR, fill=variable)) + geom_bar(stat="identity")  + theme_bw(15) + theme(aspect.ratio=1, legend.position="none") + coord_flip() + ylab("FPR") + geom_hline(yintercept=0.05, linetype="dashed") + facet_wrap(~p + sig, ncol=length(sig_array)) + xlab('')
...
fig2 = ggplot(df_AUPR, aes(variable, AUPR, fill=variable)) + theme_bw(15) + theme(aspect.ratio=1, legend.position="none") + coord_flip() + geom_bar(stat="identity") + ylab("AUPR") + facet_wrap(~p + sig, ncol=length(sig_array)) + xlab('')

grid.arrange(fig1, fig2, ncol=1)
```

 RE2C is more powerful than multiXcan for large p and high correlation between features
