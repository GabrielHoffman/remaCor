...
S_MultiXcan.reg is proposed by [Barbeira, et al, 2019](https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1007889).  It is like S_MultiXcan, but accounts for the fact that correlation between features can be low rank.

RE2C in remaCor is proposed here, but based on other work.

anova is the linear model test if there were complete GWAS data.

Simulations from a GWAS of 10K samples.  The imputed features explain 1e-4 of the variance in the phenotype in the GWAS.
...
sig_array = c(0, .3, .6, .9)
hsq = 1e-4 # fraction of variance in y explained by X
